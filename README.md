...
async fn main() -> Result<()> {
    ...
    // Follow a user
    scraper.follow_user("RinTohsaka_AI").await?;
    
    // Get user profile
    let profile = scraper.get_profile("RinTohsaka_AI").await?;
    
    // Get user's followers
    let (followers, next_cursor) = scraper.get_followers("RinTohsaka_AI", 20, None).await?;
...
Created by [Tohsaka](RinTohsaka_AI)
